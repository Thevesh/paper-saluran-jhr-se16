...
Each saluran therefore forms a contingency table with exact demographic and electoral margins but an unobserved interior---the number of registered Chinese voters in their sixties who cast a BN ballot, say. Recovering that interior is the classic ecological inference problem \cite{robinson1950,king1997}, here repeated across 14,222 saluran-election tables. The margins are administrative totals, not samples, and we verify all accounting identities linking the roll, ballots issued, valid, rejected and unreturned ballots, and candidate votes.

Finally, we exclude early-voting salurans and postal ballots. Postal ballots carry no registered-electorate margin, so no table can be built; early-voting salurans (police, military and their spouses) are a demographically and behaviourally distinct bloc (turnout 78.7\% at SE-16 against 68.6\% overall), and including them while excluding their postal counterparts would create a data-driven rather than substantively coherent sample. The excluded bloc is under 3.5\% of ballots, and including the early salurans moves no statewide estimate by more than about one point. All figures therefore describe the ordinary polling-day electorate, which is why our statewide totals differ slightly from the official ones quoted in Section~\ref{sec:context}: turnout at SE-16 was 68.6\% on this basis against an official 69.6\% (54.9\% and 75.5\% officially at SE-15 and GE-15), and BN's and PH's SE-16 shares are 59.4\% and 33.0\% against the official 59.7\% and 32.6\%.
...
The denominator is registration, not turnout, deliberately: staying home is treated as one of the things a voter can do, so a group's turnout and its party rates share one denominator, add up, and keep mobilisation and persuasion inside one accounting frame---which matters when turnout moves by twenty points between elections. Throughout, we call the register the \emph{electorate} and reserve \emph{ballots} for votes actually cast: a group's rates as shares of its electorate sum across outcomes, and its shares of ballots follow by dividing through by turnout. We report both.

Every model below rests on one identity: a saluran's rate is the average of its groups' rates, weighted by composition,
\begin{equation}
	p_{ic} \;=\; \sum_g x_{ig}\, b_{igc},
	\label{eq:identity}
\end{equation}
where $p_{ic}$ is saluran $i$'s observed rate of outcome $c$, $x_{ig}$ is group $g$'s share of saluran $i$'s electorate, and $b_{igc}$ is group $g$'s unobserved rate of $c$ in that saluran. Both $x$ and $p$ are observed. The identity does not deliver $b$; assumptions do, and the steps below audit which assumptions earn their place.
...
\subsection{Step 1: ecological regression, and where it breaks}
...
\begin{equation}
	...
	\tag{M1}\label{eq:m1}
\end{equation}
with saluran electorates $V_i$ as weights, no intercept, and $\varepsilon_{ic}$ the residual; the coefficients \emph{are} the group rates \cite{goodman1953}, and the assumption is now in the open: the \emph{same} $m$ everywhere.

It breaks. Table~\ref{tab:m1} prints the ethnic estimates in full rather than asserting the failure. Six of the 120 group-outcome estimates across the three elections fall outside their deterministic bounds, the worst putting PN support among Indian voters at $-5.2\%$ at SE-16. The failures concentrate where small parties meet groups spread thinly across the state, requiring extrapolation beyond the observed compositional support. These clipped estimates also reproduce a published breakdown of these elections whose estimator is otherwise undocumented.

\input{tab_m1}

\subsection{Step 2: a link function}
...
One corner needs handling: PN is almost absent among non-Malays, so the unpenalised likelihood sends those coefficients to $-\infty$. A weakly informative $N(0,2.5^2)$ prior on each $\beta$ \cite{gelman2008} keeps the corner finite while changing identified estimates by at most 0.034 points. Statewide, one estimate of 120 now sits outside its bounds, by 0.025 points---but a statewide estimate can respect its statewide bounds and still be impossible in individual streams, which is what the next step establishes.

\subsection{Step 3: the accounting constraint}
...
\begin{equation}
	...
	\label{eq:bounds}
\end{equation}
These sharp bounds \cite{duncandavis1953} rest only on the observed margins: the true cell count must lie within them. Their width therefore measures how much each estimate is constrained by arithmetic rather than modelling \cite{chogaines2004}, and they bind hardest where a stream is dominated by one group---which is why Johor's near-pure streams matter so much in Step 5. Table~\ref{tab:bounds} reports them statewide, for every group and election: whatever model one prefers, any account of these elections must live inside these intervals.
...
The pattern is inconsistent with a mobilisation account, because Malay turnout did not rise, and with a big-tent account, because the measurable non-Malay contributions were small. It therefore identifies net reversion within the Malay electorate. The natural interpretation is a return from PN to BN, although individual transitions and offsetting churn remain unobserved.
...
One implication is immediate, because the arrangement Johor previewed has since been extended: heading into the Negeri Sembilan state election, BN and PN have divided all 36 seats between them. Two Johor observations carry over: BN's gain was no larger where PN withdrew, so standing down added little BN was not achieving anyway; and PN's position is weaker than its headline suggests, because the seats it kept were the most favourable to it and it still lost more than two thirds of its vote there. A full pact removes the comparison that made both statements checkable.

Finally, Johor is not Malaysia: it is UMNO's heartland and PN's weakest peninsular flank. That the PN-to-BN flow can happen at scale, quickly, and across all age groups is transportable. The level of BN support is not.
...
Johor's electoral geography is unusually favourable to ecological inference, and its sorting rule is an administrative accident. Three lessons from this exercise depend on neither.

First, staying home is a choice, and should be modelled as one. Most EI applications condition on having voted, estimating how each group divided the ballots it cast---which removes the largest thing that moves between elections. Making registration the denominator instead lets a group's turnout and its party rates sum to one, keeping mobilisation and persuasion inside a single frame. Johor's Chinese electorate shows what this buys: PH's support fell 10.9 points of that electorate between GE-15 and SE-16, roughly two-thirds of it through lower turnout. Conditioning on voters would have shown only the remaining third---a 5.9-point fall in PH's ballot share that we cannot distinguish from zero---missing the finding while reporting a smaller one confidently. The distinction is not cosmetic, because the two channels imply different prospects for recovery: a party that has lost voters to abstention faces a different task from one that has lost them to a rival. Wherever participation moves between the elections compared, `Who moved?' cannot be answered without `Who showed up?'.

Second, identification is won through richer rows, not richer likelihoods. Age-by-vote tabulations are among the most-quoted numbers in electoral commentary anywhere, and wherever age composition differs by ethnicity, religion or region \emph{and} behaviour differs along the same cleavage, the marginal age gradient is confounded by construction \cite{achenshively1995,chen2026}---here enough to reverse its sign. Two things make this more than a caution. The roll carries both attributes for the same individual, so the confound can be measured rather than warned against. And the repair is not a better estimator but a finer cross-classification: our deliberately plain specification matched a hierarchical Bayesian R$\times$C model cell-for-cell on the held-out streams, while redefining a row from ten marginal groups to twenty-four joint cells cut the age-cell miss from 4.93 points to 0.98. Where the row definition omits a jointly observed confounder, a finer cross-classification should take priority over a more sophisticated likelihood---and whether one is available is a question about the administrative record, not about method. The contrast with turnout is concrete: the age gradient in BN support dissolves under decomposition, the age profile of turnout survives it. Both are age patterns in the same marginals, and only one is real.

Third, uncertainty should be measured where it can be and left unquantified where it cannot. That anything can be measured is itself administrative: wherever voters are assigned to reporting units by an observable attribute---age here, alphabet, address or registration date elsewhere---near-pure units arise in which a group's behaviour is nearly observed, and can be held out and predicted. The same sorting that endangers the constancy assumption supplies the test that detects the danger. What that test yields must then match both the quantity it decorates and its scale: our estimator predicts Chinese PN support to within a fraction of a point and Malay BN support at GE-15 to within six, so a single bar per group would misdescribe both, and the miss against an individual stream averages 5.8 points against 1.9 for the statewide rate. Errors belong to estimands, not to rows of a table, and derived quantities should inherit their components' errors rather than be scored against their own wider bounds, where component errors can cancel.

The trap is to reach for a standard error instead. A standard error asks how far an estimate would move in a different sample and shrinks as the sample grows; an EI estimate's problem is not that it was sampled but that it is unidentified, and being wrong about the behavioural assumption produces bias, which no quantity of data shrinks. Indian turnout at SE-16 carries a bootstrap standard error under three points on a quantity the margins confine only to between 1.7\% and 99.8\%. Counting 2.7 million voters rather than surveying two thousand buys precision, not accuracy: an EI estimate cannot be defended on sample size, and a significance test run on an electorate this large will certify almost anything. Where no validatable unit exists, then, the honest report is no interval at all---so Indian and Other voters here carry point estimates without error bars, flagged as not citable. The Indian series in Table~\ref{tab:results} is the cautionary exhibit: at face value it shows turnout doubling and halving across four years, artefacts of an unidentified parameter.

\section{Conclusion}
\label{sec:conclusion}

The July 2026 Johor landslide, decomposed against validated group-level estimates, was three stories and a non-story. A 28-point net reversion of Malay voters from PN to BN, robust to PN's stand-down in 23 seats. That reversion undoing a green wave which had been steeply generational: young Malays carried it in 2022, their return in 2026 was largest, and Malay politics flattened across age. The predominant demobilisation of Chinese voters, whose loyalty to PH largely persisted among those voting while their participation fell. And the non-story: the celebrated youth swing to BN, which dissolves into the ethnic composition of Johor's age pyramid.

Around these findings the paper offers two durable artefacts: an open, reproducible pipeline that turns two published margins into bounded, validated estimates of who voted for whom, extensible to any Malaysian election; and a demonstration that the administrative sorting of voters into polling streams, a threat to ecological inference when ignored, becomes a validation resource when exploited. The estimates are what a reader will quote; the second artefact is the one we would rather they took away, because its lesson---that identification is won through richer rows rather than richer likelihoods, and that the units an administration built for its own reasons will tell you which---does not depend on Malaysia at all. As Malaysia approaches its 16th general election with the same roll, the same salurans, and the same contending coalitions, both the estimates and the machinery are on the table.

\section*{Data and code availability}
All inputs are public, from the from the Malaysian Election Corpus \cite{meco1}. The complete pipeline---data construction, estimation, validation, presentation---is available for replication \href{https://github.com/Thevesh/paper-saluran-jhr-se16}{here}.

\section*{Funding}
This research did not receive any specific grant from any entity in the public, commercial, or not-for-profit sectors.

\section*{Declaration of competing interests}

TT does not declare any competing interests.

OKM is a member of the Democratic Action Party (DAP)---a component party of PH, one of the coalitions whose support this paper estimates---and served as its Member of Parliament for Serdang (2013--2018) and subsequently Bangi (2018--2022). He left frontline politics in 2023 and is currently an Adjunct Professor at Taylor's University.
